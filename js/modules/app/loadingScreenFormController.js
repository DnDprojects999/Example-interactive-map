...
export function createLoadingScreenFormController(options) {
  const {
    els,
    state,
    persistWorldInfo,
  ...
  function syncStaticLabels() {
    ...
    els.loadingEditorFieldArtworkLabel.textContent = getUiText(state, "prompt_loading_image");
    els.loadingEditorAddFlavorLineButton.textContent = getUiText(state, "loading_editor_add_line");
    els.loadingEditorUploadImageButton.textContent = getUiText(state, "loading_editor_upload_image");
    els.loadingEditorClearImageButton.textContent = getUiText(state, "loading_editor_clear_image");
  ...
  function renderArtworkPreview() {
    if (!els.loadingEditorArtworkPreview) return;
    const normalized = String(artworkValue || "").trim();
    if (!normalized) {
      els.loadingEditorArtworkPreview.hidden = true;
      els.loadingEditorArtworkPreview.style.backgroundImage = "none";
      return;
    }
    els.loadingEditorArtworkPreview.hidden = false;
    els.loadingEditorArtworkPreview.style.backgroundImage = `url("${normalized.replace(/"/g, "\\\"")}")`;
  }
  ...
  function fillForm() {
    ...
    artworkValue = String(localizedWorldInfo.loadingImageUrl || "").trim();
  ...
  function applyFormValues() {
    ...
    setLocalizedValue(state.worldData, "loadingImageUrl", artworkValue, state);
    setLocalizedValue(
      state.worldData,
      "loadingFlavorLines",
  ...
  async function uploadImage(file) {
    ...
    artworkValue = await readFileToDataUrl(file);
    renderArtworkPreview();
    if (!isOpen()) {
      setLocalizedValue(state.worldData, "loadingImageUrl", artworkValue, state);
      persistWorldInfo?.();
  ...
  function clearImage() {
    ...
    renderArtworkPreview();
    if (!isOpen()) {
      setLocalizedValue(state.worldData, "loadingImageUrl", "", state);
      persistWorldInfo?.();
    }
  }
